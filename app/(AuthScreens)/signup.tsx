...
// import { createUserWithEmailAndPassword, sendEmailVerification, fetchSignInMethodsForEmail, getAuth, signInWithEmailAndPassword } from 'firebase/auth';
// import axios, { AxiosError, formToJSON } from 'axios'; // Axios for API requests
...
//   //       // Call the API to verify and activate the user in your backend
...
//     if (name === 'email') {
//       // Ensure the email is stored in lowercase when the user finishes typing (on blur)
//       setFormData((prevFormData) => ({
//         ...prevFormData,
...
//     // Validate Password
//     formErrors.password = password.match(passwordRegex)
//       ? ''
...
//           return;
//         }
//         // Call your backend to save the user to your MongoDB or other databases
...
//          Alert.alert('Exist','Account already exist please verify');
...
//           // Alert.alert(response.data.message)
//           // Alert.alert('Success','Account created successfully, please verify email')
//           // await sendEmailVerification(user);
//           handleEmailVerification(user.uid);
...
//         // Redirect to the login screen or dashboard
...
// //         } else if (response.status === 201) {
// //           // Store timestamp of signup attempt
...
//       if (response.status === 200) {
//         Alert.alert('Success', 'Your email is verified, and your account is activated.');
//         router.navigate({ pathname: '/(AuthScreens)/login', params: { email: '' } });
//       } else {
//         Alert.alert('Error', response.data.message || 'Failed to verify email.');
//       }
//     } catch (error) {
...
const handleSignup = async () => {
  ...
  if (validateForm()) {
    try {
      ...
      // Call backend to save user details
      const response = await axios.post('http://192.168.0.109:5000/api/auth/signup', {
        username: formData.username,
        email: user.email,
        phone: formData.phone,
        password: formData.password,
  ...
  const handleEmailVerification = async (uid:string) => {
    try {
      const response = await axios.post('http://192.168.0.109:5000/api/auth/verifyUserEmail', { uid });
